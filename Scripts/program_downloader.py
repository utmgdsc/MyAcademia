...
parser.add_argument('cookie', type=str,
                    help="cookie from a valid Degree Explorer session. To obtain this, log into DE with your UofT credentials, then copy the cookie from the Network tab of Chrome Devtools")
parser.add_argument('--p_jsons_dir', type=str,
                    help="path to directory to store downloaded program JSONs. default: ./program_data",
                    default="./program_data", metavar='dir')
parser.add_argument('--p_cc_ids_file', type=argparse.FileType('a'),
                    help="path to ASCII file to store course categories from downloaded program JSONs. default: ./program-course-category-ids.txt",
                    default="./program-course-category-ids.txt", metavar='file')
if __name__ == "__main__":
    ...
    for line in sys.stdin:
        ...
        if (studyAreaNum != currentStudyArea):
            r = requests.post(
                "https://degreeexplorer.utoronto.ca/degreeExplorer/rest/dxPlanner/resetPrograms?tabIndex=0",
                headers=resetProgramsPOSTHeader)
        ...
        thisProgramObj = list(filter(lambda programDataObj: programDataObj["postCode"] == programID,
                                     r.json()["timelineStatus"]["allPostAssessments"]))[0]
        ...
        for detailAssessment in thisProgramObj["detailAssessments"]:
            for requisiteItem in detailAssessment["requirement"]["requisiteItems"]:
                ...
                if not allCoursesRe.match(code) and not allProgramsRe.match(code) and not requirementRe.match(
                        code) and code != "":
    ...
    r = requests.post("https://degreeexplorer.utoronto.ca/degreeExplorer/rest/dxPlanner/resetPrograms?tabIndex=0",
                      headers=resetProgramsPOSTHeader)
